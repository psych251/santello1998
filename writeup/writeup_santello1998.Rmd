...
The above procedure was followed precisely and the list of objects presented in Table 1 of the paper was used without modification.
...
####Import data
d1 = read.csv("", to.data.frame=T)
d2 = read.csv("", to.data.frame=T)
d3 = read.csv("", to.data.frame=T)
d4 = read.csv("", to.data.frame=T)
d5 = read.csv("", to.data.frame=T)
...
handpose_long_d = handpose_wide_raw %>%
  pivot_longer(cols=-c("Participant id", "Object", "Trial"),
               names_to='Joint', 
               values_to='Angle_degrees')
handpose_long %>% 
  group_by(Participant_id, Object) %>%
...
handpose_wide_processed = handpose_long %>%
  pivot_wider(handpose_long, id_cols = -c(Mean_angle_degrees, Angle_degrees), names_from = Joint, values_from = Recentered_angle_degrees)
