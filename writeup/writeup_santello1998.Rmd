...
The joint angles are recorded using a Unity app written for this purpose, with the hand-tracking feature built into the Windows Mixed Reality Toolkit (MRTK). A C# script was written to record joint positions when the participant presses the spacebar on a keyboard, compute joint angles from the positions, and log them to a text file as comma-separated values. The script is available at [https://github.com/psych251/santello1998/blob/main/code/hand_tracking/Assets/logJointData.cs](https://github.com/psych251/santello1998/blob/main/code/hand_tracking/Assets/logJointData.cs). 

The data was placed into Excel spreadsheets by the following procedure:
1. Before data collection: populating the ParticipantID column manually, and randomizing a list of obejcts with three instances of each object in the second column; repeating each instance 15 times so that the joint angles can be pasted after data collection;
...
#handpose_wide_processed = handpose_wide_processed_uncentered %>% 
#  mutate(Recentered_angle_degrees = select(handpose_wide_processed_uncentered, -c(ParticipantID, Object))-colMeans(select(handpose_wide_processed_uncentered,-c(ParticipantID, Object))))
...
#handpose_wide_transposed_all <- handpose_wide_processed_all %>%
  #group_by(ParticipantID) %>%
  #pivot_longer(-c(ParticipantID, Joint), names_to = "Object", values_to = "Mean_angle_degrees_centered")%>%
  #pivot_wider(names_from = "Joint", values_from = "Mean_angle_degrees_centered")%>%
  #ungroup()
...
njoints = length(unique(handpose_long$Joint))
nobj = length(unique(handpose_long$Object))
npart = length(unique(handpose_long$ParticipantID))

#outputs to compare
object_plots <- vector(mode = "list", length = npart)
variance_plots <- vector(mode = "list", length = npart)
pva_list <- vector(mode = "list", length = npart)
#coeffs_list <- data.frame(Participant = rep(as.factor(c(1:npart)), each=nobj), Object =  rep(as.character(c(1:nobj)), times=npart), PC1 = rep(c(1:nobj), times=npart), PC2 = rep(c(1:nobj), times=npart))
coeffs_list <- data.frame(Participant = rep(as.factor(c(1:npart)), each=nobj), Object =  rep(as.character(NA), times=npart*nobj), PC1 = rep(NA, times=npart*nobj), PC2 = rep(NA, times=npart*nobj))

#do PCA for each participant
for(i in 1:npart){
handpose_wide_processed = as.data.frame(datalist[[i]])
...
#eigenvalues of covariance matrix, just to check that prcomp is doing the same thing as authors did
...
#pca_handpose = prcomp(handpose_wide_processed, scale=FALSE)
#summary(pca_handpose)
#pca_handpose$sd^2

#array for objects in PC basis
...
for (objnumber in 1:nobj_i){
  ...
  #testvec <- testvec -sum(pca_handpose$x[,pcnumber]*handpose_wide_processed$Apple)/sum(pca_handpose$x[,pcnumber]*pca_handpose$x[,pcnumber])*pca_handpose$x[,pcnumber]
...
#check -- this should be close to zero in each entry
...
#pr.var <- pca_handpose$sd^2
# Variance explained by each principal component: pve
pva_list[[i]] <- eigvals / sum(eigvals)

}

#plot(pva_list[[i]], xlab = "Principal Component", ylab = "Proportion of Variance Explained", ylim = c(0, 1), type = "b")


xValue <- 1:njoints
#yValue <- cumsum(pva_list[[1]])
...
#vp <- plot(cumsum(pve), xlab = "Principal Component",
